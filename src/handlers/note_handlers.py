...
class Note(CRUDMixin):
    ...
    def add_tag(self, value: str):
        return self._add_item(self._tags, Tag, value)

    def change_tag(self, old_value: str, new_value: str):
        return self._change_item(self._tags, Tag, old_value, new_value)

    def remove_tag(self, value: str):
        self._remove_item(self._tags, Tag, value)

    def find_tag(self, value: str):
        return self._find_item(self._tags, Tag, value)
    ...
    def __str__(self):
        ...
        return f"{self.text} [{tags}]" if tags else self.text
...
def remove(self, field: str, value):
 ...
 # Auxiliary Business Logic for Birthdays
    ...
    def get_congratulation_date(self) -> datetime | None:
        ...
        date = self.birthday.next_birthday()
    ...
    def __str__(self):
        ...
        parts += [
            f"{field.capitalize()}: " +
            ", ".join(item.value for item in getattr(self, f"_{field}").values())
            for field in self.COLLECTIONS
            if getattr(self, f"_{field}")
        ]
...
class AddressBook(CRUDMixin):
    ...
    def get_or_create_record(self, name: str) -> Record:
        key = self._key(name)
        return self._records.setdefault(key, Record(Name(name)))

    def add_record(self, record: Record):
        self._records[record.name.value.lower()] = record    

    def find(self, name: str):
        return self._records.get(self._key(name))

    def delete(self, name: str):
        self._records.pop(self._key(name), None)
    ...
    def get_upcoming_birthdays(self, days_ahead: int = 7) -> list[dict]:
        ...
        result = []
        for record in self:
            congratulation = record.get_congratulation_date()

            if not congratulation:
                continue

            delta = (congratulation - today).days

            if 0 <= delta <= days_ahead:
                result.append({
                    "name": record.name.value,
                    "congratulation_date": congratulation.strftime("%d.%m.%Y")
                })
    ...
    def remove_tag_index(self, record: Record, note: Note, tag: str):
        items = self._tag_index.get(tag)
        if not items:
            return

        self._tag_index[tag] = [
            (r, n) for r, n in items if (r, n) != (record, note)
        ]
